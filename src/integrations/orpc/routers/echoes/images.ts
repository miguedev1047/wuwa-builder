    ...
    .handler(async ({ input }) => {
      ...
      try {
        ...
        const [assetCreated] = await db
          .insert(echoAssetsTable)
          .values({ echo_id: entityId, key: 'none', order })
          .returning({ id: echoAssetsTable.id })
        ...
        await db
          .update(echoAssetsTable)
          .set({ key })
          .where(eq(echoAssetsTable.id, assetId))
  ...
  delete: protectedProcedure
    ...
    .handler(async ({ input }) => {
      ...
      try {
        ...
        await db.delete(echoAssetsTable).where(eq(echoAssetsTable.id, id))
